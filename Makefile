...
TWEAK_NAME = nojbhookoffsetpatch

nojbhookoffsetpatch_CFLAGS = -fobjc-arc -w
nojbhookoffsetpatch_CCFLAGS = -std=c++11 -fno-rtti -fno-exceptions -DNDEBUG

ifeq ($(IGNORE_WARNINGS),1)
  nojbhookoffsetpatch_CFLAGS += -w
  nojbhookoffsetpatch_CCFLAGS += -w
endif


nojbhookoffsetpatch_FILES = Tweak.xm $(MENU_SRC) $(KITTYMEMORY_SRC) $(SCLALERTVIEW_SRC)

nojbhookoffsetpatch_LIBRARIES += substrate
nojbhookoffsetpatch_LDFLAGS += -L./
nojbhookoffsetpatch_LDFLAGS += -lz -stdlib=libc++ -ldobby_fixed

nojbhookoffsetpatch_FRAMEWORKS = $(PROJ_COMMON_FRAMEWORKS)
# GO_EASY_ON_ME = 1
