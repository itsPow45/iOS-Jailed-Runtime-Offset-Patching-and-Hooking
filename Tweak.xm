...
void FirstLookUpdate(void *instance) {
  ...
  if([switches isSwitchOn:NSSENCRYPT("FOV")]) {
   *(float *)((uint64_t) instance + 0x30) = [[switches getValueFromSwitch:NSSENCRYPT("FOV")] floatValue];;
  } else {
    *(float *)((uint64_t) instance + 0x30) = 60;
  }
...
void setup() {
    ...
    HOOK(0x1C41544, FirstLookUpdate, old_FirstLookUpdate);
    
  [switches addSliderSwitch:NSSENCRYPT("FOV")
...
void setupMenu() {
  ...
  menu = [[Menu alloc]  
            initWithTitle:NSSENCRYPT("Jailed Offset Patch And Hooking Runtime")
            titleColor:[UIColor whiteColor]
            titleFont:NSSENCRYPT("Copperplate-Bold")
            credits:NSSENCRYPT("Jailed Offset Patch And Hooking Runtime")
